...
const MonsterHurt = styled.div<MonsterHurtProps>`
    ...
    @keyframes playHurt {
        from { background-position: 0 0; }
        to { background-position: ${({ steps }) => -280 * steps}px 0; }
    }
`;

export function MonsterImage ({fullSteps, state, idleUrl, hurtUrl, onHurtAnimationEnd, hurtDuration, hurtSteps}: {fullSteps: number, state: "idle" | "hurt", idleUrl: string, hurtUrl: string, onHurtAnimationEnd?: () => void, hurtDuration: number, hurtSteps: number}) {
    return (
        <div className={state}>
            {state == "idle" && <MonsterIdle idleUrl={idleUrl} />}
            {state == "hurt" && <MonsterHurt fullSteps={fullSteps} hurtUrl={hurtUrl} onAnimationEnd={onHurtAnimationEnd} duration={hurtDuration} steps={hurtSteps} />}
        </div>)
}